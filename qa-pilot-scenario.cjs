...
(async () => {
  ...
  try {
    for (const viewport of [{ width: 1440, height: 1000 }, { width: 768, height: 1024 }, { width: 390, height: 844 }]) {
      ...
      await page.locator('.profile-button:visible').click();
      await page.locator('[data-action="navigate"][data-view="shared-device"]:visible').first().click();
      await page.locator('[data-action="select-shared-user"][data-id="' + report.scenario.employeeIds[0] + '"]').click();
      await page.locator('form[data-form="shared-pin"] input[name="pin"]').fill('111111');
      await page.locator('form[data-form="shared-pin"] button.primary').click();
      await page.getByRole('heading', { name: /Meine Woche · KW 38/ }).waitFor();
      assert.equal(await page.locator('.my-week').getByText('Projekt Stephan (Demo)').count() > 0, true);
