...
(function exposeExportCenter() {
  ...
  function render(db) {
    ...
    return headBlock() +
      '<section class="card card-pad export-controls"><div><label>Baustelle / Bau-Nr.<select id="mm-export-project">' + projectOptions + '</select></label></div><div><label>Mitarbeiter<select id="mm-export-employee">' + employeeOptions + '</select></label></div><div><label>Planungswoche<select id="mm-export-week">' + weekOptions + '</select></label></div><div><label>Kalenderjahr<select id="mm-export-year">' + yearOptions + '</select></label></div></section>' +
      '<section class="section"><div class="section-title"><h2>Besonders wichtige Übergangsexporte</h2><span class="meta">an den heutigen Unterlagen orientiert</span></div><div class="export-grid mm-export-grid">' +
        ...
        exportCard('Urlaubsplaner', 'Jahresmatrix aus genehmigten Demo-Anträgen; keine Resturlaubs- oder Anspruchsberechnung.', [button('Jahresplan als XLSX', 'xlsx-leave', 'primary')], 'XLSX') +
        exportCard('Bauliste 2026 / Nachkalkulation', 'Hauptliste und synthetische Projektblätter mit quellnahen Formelverknüpfungen; Kalkulationssatz ist ein Demo-Wert.', [button('XLSX herunterladen', 'xlsx-calculation', 'primary')], 'XLSX') +
        exportCard('Projekt-Unterkonto', 'Projektblatt mit Wochenstunden, Rechnungen und Kosten-Summen an beobachteten Zellpositionen.', [button('XLSX herunterladen', 'xlsx-project', 'primary')], 'XLSX') +
  ...
  function handleAction(target, db, hooks) {
    ...
    if (action === 'xlsx-leave') saveXlsx('Maler-Meyer-Demo-Urlaubsplaner-' + pick.year + '.xlsx', window.MMSourceWorkbooks.leaveCalendar(db, pick.year));
    if (action === 'xlsx-calculation') saveXlsx('Maler-Meyer-Demo-Bauliste-2026.xlsx', window.MMSourceWorkbooks.calculation(db));
    if (action === 'xlsx-project') saveXlsx('Maler-Meyer-Demo-Projekt-' + pick.site + '.xlsx', window.MMSourceWorkbooks.project(db, pick.site));
  ...
  function openPrint(db, template, pick) {
    const views = {
      timesheet: timesheetHtml, 'timesheet-confirmed': confirmedTimesheetHtml, 'extra-confirmation': extraConfirmationHtml, planning: planningHtml, 'invoice-list': invoiceListHtml,
  ...
  function confirmedTimesheetHtml(db, pick) {
    const snapshot = db.weeklySnapshots.find(function (item) { return item.id === pick.snapshotId; });
    if (!snapshot) return timesheetHtml(db, pick);
    const content = snapshot.content;
    const rows = content.days.map(function (day) {
      return '<tr><td class="bold">' + h(day.day) + '</td><td>' + h(day.date) + '</td><td>' + h(day.start) + '</td><td>' + h(day.break) + '</td><td>' + h(day.end) + '</td><td>' + h(day.travel) + '</td><td>' + h(project(db, day.site).name) + '</td><td>' + h(day.site) + '</td><td>' + h(day.issue || 'vollständig') + '</td></tr>';
    }).join('');
    const approval = db.weeklyApprovals.find(function (item) { return item.snapshotId === snapshot.id; });
    const status = approval ? 'Büro/Admin freigegeben' : 'Vom Mitarbeiter bestätigt';
    const signature = snapshot.signatureDataUrl ? '<img class="drawn-signature" src="' + snapshot.signatureDataUrl + '" alt="Gezeichnete Demo-Unterschrift">' : '<span class="muted">Ohne gezeichnete Unterschrift bestätigt</span>';
    const old = snapshot.supersededByVersion ? '<p class="superseded">Nicht mehr aktueller Stand – durch Version ' + snapshot.supersededByVersion + ' ersetzt</p>' : '';
    return { title: 'Bestätigter Arbeitszeitnachweis ' + content.week + ' · Version ' + content.version, orientation: 'landscape', body: '<main class="page timesheet-doc">' + docHeader('Vorlage zur Dokumentation der täglichen Arbeitszeit') + old + '<div class="confirmation-band"><span><small>Mitarbeiter</small><strong>' + h(content.employeeName) + '</strong></span><span><small>Kalenderwoche</small><strong>' + h(content.week) + '</strong></span><span><small>Version / Status</small><strong>Version ' + content.version + ' · ' + h(status) + '</strong></span><span><small>Bestätigt</small><strong>' + h(snapshot.confirmedAt) + '</strong></span></div><table><thead><tr><th>Tag</th><th>Datum</th><th>Beginn</th><th>Pause</th><th>Ende</th><th>Fahrt roh</th><th>Baustelle</th><th>Bau-Nr.</th><th>Vollständigkeit</th></tr></thead><tbody>' + rows + '</tbody></table><p class="small"><strong>Snapshot-ID:</strong> ' + h(snapshot.id) + ' · <strong>Demo-Hash:</strong> ' + h(snapshot.contentHash) + '</p><p class="small">Fahrt wird ausschließlich als Rohangabe gezeigt. Keine Lohn- oder Überstundenbewertung.</p><div class="signatures"><div><strong>Bestätigung durch ' + h(snapshot.confirmedBy) + '</strong>' + signature + '<small>Bestätigung mit gezeichneter Unterschrift ist keine qualifizierte elektronische Signatur.</small></div><div></div><div>' + (approval ? '<strong>Freigegeben durch ' + h(approval.approvedBy) + '</strong><br><small>' + h(approval.approvedAt) + '</small>' : '<span class="muted">Büro/Admin-Freigabe noch offen</span>') + '</div></div></main>' };
  }

  function extraConfirmationHtml(db, pick) {
    const record = db.extraConfirmations.find(function (item) { return item.id === pick.confirmationId; });
    if (!record) return { title: 'Bestätigung Zusatzarbeit', body: '<main class="page"><p>Bestätigungsdatensatz nicht gefunden.</p></main>' };
    const signature = record.signatureDataUrl ? '<img class="drawn-signature" src="' + record.signatureDataUrl + '" alt="Gezeichnete Demo-Unterschrift">' : '<span class="muted">Ohne gezeichnete Unterschrift bestätigt</span>';
  ...
  function stylesXml(){return'<?xml version="1.0" encoding="UTF-8"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><numFmts count="4"><numFmt numFmtId="164" formatCode="#,##0.00 [$€-407];[Red]-#,##0.00 [$€-407];-"/><numFmt numFmtId="165" formatCode="0.00&quot; h&quot;"/><numFmt numFmtId="166" formatCode="dd.mm.yyyy"/><numFmt numFmtId="167" formatCode="dd"/></numFmts><fonts count="4"><font><sz val="10"/><name val="Arial"/></font><font><b/><sz val="14"/><name val="Arial"/><color rgb="FF17324D"/></font><font><b/><sz val="10"/><name val="Arial"/><color rgb="FFFFFFFF"/></font><font><b/><sz val="10"/><name val="Arial"/><color rgb="FF17324D"/></font></fonts><fills count="5"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF17324D"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFD9EAD3"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFFFE8A3"/></patternFill></fill></fills><borders count="2"><border/><border><left style="thin"><color rgb="FF777777"/></left><right style="thin"><color rgb="FF777777"/></right><top style="thin"><color rgb="FF777777"/></top><bottom style="thin"><color rgb="FF777777"/></bottom></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="10"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="2" fillId="2" borderId="1" xfId="0" applyAlignment="1"><alignment horizontal="center" vertical="center" wrapText="1"/></xf><xf numFmtId="0" fontId="3" fillId="3" borderId="1" xfId="0"/><xf numFmtId="164" fontId="0" fillId="0" borderId="1" xfId="0" applyNumberFormat="1"/><xf numFmtId="165" fontId="0" fillId="0" borderId="1" xfId="0" applyNumberFormat="1"/><xf numFmtId="0" fontId="0" fillId="4" borderId="1" xfId="0"/><xf numFmtId="166" fontId="0" fillId="0" borderId="1" xfId="0" applyNumberFormat="1"/><xf numFmtId="167" fontId="0" fillId="0" borderId="1" xfId="0" applyNumberFormat="1"/><xf numFmtId="0" fontId="0" fillId="0" borderId="1" xfId="0"/></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>';}
  function sheetXml(sheet){const rows=sheet.rows||[];const cols=(sheet.widths||[]).map(function(w,i){return'<col min="'+(i+1)+'" max="'+(i+1)+'" width="'+w+'" customWidth="1"/>';}).join('');const body=rows.map(function(row,ri){return'<row r="'+(ri+1)+'" ht="'+(ri===0?24:ri===1?34:18)+'" customHeight="1">'+row.map(function(raw,ci){const obj=raw&&typeof raw==='object'&&Object.prototype.hasOwnProperty.call(raw,'v')?raw:{v:raw,s:ri===0?1:ri===1?2:0,f:''};const ref=column(ci+1)+(ri+1);const style=obj.s||0;if(obj.f){const textResult=typeof obj.v==='string';return'<c r="'+ref+'" s="'+style+'"'+(textResult?' t="str"':'')+'><f>'+xml(obj.f)+'</f><v>'+(textResult?xml(obj.v):Number(obj.v||0))+'</v></c>';}if(typeof obj.v==='number')return'<c r="'+ref+'" s="'+style+'"><v>'+obj.v+'</v></c>';return'<c r="'+ref+'" s="'+style+'" t="inlineStr"><is><t xml:space="preserve">'+xml(obj.v==null?'':obj.v)+'</t></is></c>';}).join('')+'</row>';}).join('');const pane=sheet.freeze?'<sheetViews><sheetView workbookViewId="0"><pane xSplit="'+(sheet.freeze.cols||0)+'" ySplit="'+(sheet.freeze.rows||0)+'" topLeftCell="'+column((sheet.freeze.cols||0)+1)+((sheet.freeze.rows||0)+1)+'" state="frozen"/></sheetView></sheetViews>':'<sheetViews><sheetView workbookViewId="0"/></sheetViews>';return'<?xml version="1.0" encoding="UTF-8"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">'+pane+'<cols>'+cols+'</cols><sheetData>'+body+'</sheetData><pageSetup orientation="'+(sheet.landscape?'landscape':'portrait')+'" paperSize="9" fitToWidth="1" fitToHeight="0"/><pageMargins left="0.3" right="0.3" top="0.5" bottom="0.5" header="0.2" footer="0.2"/></worksheet>';}
