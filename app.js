...
function loadDb() {
  try {
    ...
    if (saved && saved.version === window.DEMO_DATA_VERSION && saved.data) {
      if (saved.data.sharedDevice) saved.data.sharedDevice.autoLockMinutes = 5;
      return saved.data;
    }
  } catch (_) {}
...
function canDecideLeave() { return ui.role === 'management' || ui.role === 'management2'; }
...
function scheduleSharedLock() {
  ...
  const minutes = db.sharedDevice && db.sharedDevice.autoLockMinutes || 5;
  sharedLockTimer = setTimeout(function () { lockSharedDevice('Demo-Autosperre nach ' + minutes + ' Minuten'); }, minutes * 60 * 1000);
}
...
function siteFolder(item) {
  ...
  return `<div class="folder-detail"><div class="folder-head"><div><span class="eyebrow">Digitale Baustellenmappe</span><h2>Bau-Nr. ${item.number} · ${esc(item.name)}</h2><p>${esc(item.customer)}</p></div><button class="secondary" data-action="close-site">Schließen</button></div><div class="detail-grid">
    <section class="detail-block"><h3>Stammdaten</h3><dl class="facts"><dt>Adresse</dt><dd>${esc(item.address)}</dd><dt>Ansprechpartner</dt><dd>${esc(item.contact)}</dd>${item.siteLead ? `<dt>Baustellenleitung</dt><dd>${esc(item.siteLead)}</dd>` : ''}<dt>Zugang</dt><dd>${esc(item.access)}</dd><dt>Zeitraum</dt><dd>${esc(item.dates)}</dd>${item.description ? `<dt>Leistungsumfang</dt><dd>${esc(item.description)}</dd>` : ''}${isOfficeRole() ? `<dt>Interner Bearbeiter</dt><dd>${esc(item.manager || 'Noch offen')}</dd><dt>Abrechnungshinweis</dt><dd>${esc(item.invoice)}</dd>${item.internalNote ? `<dt>Interne Notiz</dt><dd>${esc(item.internalNote)}</dd>` : ''}` : ''}</dl></section>
...
function renderLeave() {
  ...
  return `${head('Urlaub', isOfficeRole() ? 'Anträge prüfen und Auswirkungen in der Planung sehen' : 'Digital beantragen und Status verfolgen')}
    ${assumption('Laut Vorab-Fragebogen entscheiden die beiden Geschäftsführungs-Demokonten; Büro sieht die Anträge. Produktive Rechte müssen serverseitig umgesetzt werden. Resturlaub und Berechnung von Sonderfällen bleiben offen.')}
...
function renderSharedDevice() {
  ...
  return `${head('Gemeinsames Firmen-iPad', 'Benutzerwahl und persönlicher 6-stelliger Demo-PIN')}
    <p class="legal-note"><strong>Bedienungssimulation:</strong> Dies ist keine echte Authentifizierung. Offline-Entsperrung auf registrierten Geräten ist gewünscht; ihre sichere Umsetzung ist noch zu entwerfen. Produktiv sind PIN-Hash, Fehlversuchsbegrenzung, Gerätezulassung, Audit, sichere Sessions und Geräteentzug erforderlich. Ein vergessener PIN benötigt einen sicheren Büro-Hilfe-/Rücksetzablauf.</p>
    <section class="card shared-device-card"><div class="shared-device-head"><div><strong>${esc(db.sharedDevice.deviceName)}</strong><small>Vorab-Fragebogen · automatische Sperre nach 5 Minuten; hier nur simuliert</small></div>${badge('Demo-Gerät', 'warn')}</div>
      ${!selected ? `<h2>Wer arbeitet jetzt am Gerät?</h2><div class="shared-user-grid">${activeEmployees().filter(function (item) { return window.MMAdvanced.hasDemoPin(item.id, db); }).map(function (item) { return `<button data-action="select-shared-user" data-id="${item.id}"><span class="avatar">${esc(item.name.charAt(0))}</span><span><strong>${esc(item.name)}</strong><small>${esc(item.job)}</small></span></button>`; }).join('')}</div>` : `<form data-form="shared-pin"><button type="button" class="compact-link" data-action="shared-device-switch">← Andere Person wählen</button><div class="shared-selected"><span class="avatar">${esc(selected.name.charAt(0))}</span><span><strong>${esc(selected.name)}</strong><small>${esc(selected.job)}</small></span></div><label>Persönlicher 6-stelliger Demo-PIN<input name="pin" inputmode="numeric" pattern="[0-9]{6}" maxlength="6" autocomplete="off" required ${cooldown ? 'disabled' : ''} aria-label="6-stelliger Demo-PIN"></label><button class="primary full-button" ${cooldown ? 'disabled' : ''}>Eigene Ansicht öffnen</button><p class="meta">Synthetischer Test-PIN für ${esc(selected.name)}: ${esc(window.MMAdvanced.demoPinHint(selected.id, db))}</p>${cooldown ? `<p class="problem-text">Demo-Cooldown aktiv. Bitte kurz warten.</p>` : ui.sharedPinFailures ? `<p class="problem-text">${ui.sharedPinFailures} von 5 Fehlversuchen.</p>` : ''}</form>`}
...
function renderAdmin() {
  const suggestedNumber = window.MMProjectAdmin.nextNumber(db.sites, new Date().getFullYear());
  ...
  return `${head('Verwaltung', 'Umfangreiche Betriebsverwaltung für Geschäftsführung und Büro')}
    ...
    <div class="admin-form-grid section">
      <details class="card card-pad admin-create" open><summary>Neues Projekt anlegen</summary><form data-form="admin-project-create"><div class="form-grid"><label>Bau-Nr.<input name="number" required pattern="[0-9]{2}-[0-9]{3}" value="${esc(suggestedNumber)}" placeholder="z. B. 27-001"><small>Vorschlag: höchste vorhandene Nummer dieses Kalenderjahres + 1. Vor dem Speichern änderbar.</small></label><label>Projektname<input name="name" required placeholder="z. B. Wohnhaus Regenbogen"></label><label>Kunde / Auftraggeber<input name="customer" required placeholder="vollständig erfunden"></label><label>Adresse<input name="address" required placeholder="vollständig erfunden"></label><label>Ansprechpartner<input name="contact" required placeholder="synthetischer Kontakt"></label><label>Interner Bearbeiter<input name="manager" required value="${esc(profile().name)}"></label><label class="full">Erste Aufgaben<textarea name="tasks" required placeholder="Eine Aufgabe pro Zeile"></textarea></label></div>${window.MMProjectAdmin.renderDetails(null, null, esc)}<button class="primary full-button">Projekt anlegen</button><p class="meta">Die Bau-Nr. muss eindeutig sein. Nur synthetische Daten eingeben. Vergabe und Änderbarkeit im Echtbetrieb sind noch fachlich/technisch zu klären.</p></form></details>
    ...
    <section class="section"><div class="section-title"><div><h2>Projektstammdaten</h2><p>Bearbeiten oder archivieren, ohne verbundene Vorgänge zu löschen.</p></div><span class="meta">${db.sites.filter(function (item) { return item.active !== false; }).length} aktiv · ${db.sites.filter(function (item) { return item.active === false; }).length} archiviert</span></div><div class="list">${db.sites.map(function (item) { const account = db.projectAccounts && db.projectAccounts.find(function (entry) { return entry.site === item.number; }); return `<details class="card admin-record"><summary><span><strong>Bau-Nr. ${item.number} · ${esc(item.name)}</strong><small>${item.active === false ? 'Archiviert · Historie erhalten' : esc(item.customer)}</small></span>${badge(item.active === false ? 'Archiviert' : 'Aktiv', item.active === false ? 'problem' : 'ok')}</summary><form class="form-grid" data-form="admin-project-edit" data-id="${item.number}"><label>Projektname<input name="name" required value="${esc(item.name)}"></label><label>Kunde<input name="customer" required value="${esc(item.customer)}"></label><label>Adresse<input name="address" required value="${esc(item.address)}"></label><label>Ansprechpartner<input name="contact" required value="${esc(item.contact)}"></label><label>Interner Bearbeiter<input name="manager" value="${esc(item.manager || '')}"></label><label class="full">Aufgaben · je Zeile<textarea name="tasks">${esc((item.tasks || []).join('\n'))}</textarea></label>${window.MMProjectAdmin.renderDetails(item, account, esc)}<button class="secondary">Änderungen speichern</button><button class="${item.active === false ? 'primary' : 'danger-button'}" type="button" data-action="${item.active === false ? 'reactivate-project' : 'archive-project'}" data-id="${item.number}">${item.active === false ? 'Reaktivieren' : 'Archivieren'}</button><p class="meta full">Die Bau-Nr. bestehender Projekte bleibt hier unverändert, damit verbundene Vorgänge nicht still umbenannt werden.</p></form></details>`; }).join('')}</div></section>
...
app.addEventListener('submit', async function (event) {
  ...
  if (form.dataset.form === 'leave-decision') {
    ...
    if (!request || request.status !== 'REQUESTED' || !['APPROVED', 'REJECTED'].includes(decision)) return;
    if (!canDecideLeave()) { toast('Die endgültige Urlaubsentscheidung ist der Geschäftsführung vorbehalten.'); return; }
    ...
    audit('LEAVE_DECIDED', request.id, 'Urlaubsantrag ' + leaveStatusText(decision).toLowerCase(), 'Beantragt', leaveStatusText(decision), request.decisionReason + ' · produktive Rechte serverseitig prüfen');
  ...
  if (form.dataset.form === 'shared-pin') {
    ...
    if (!employee || !window.MMAdvanced.checkDemoPin(employee.id, values.get('pin'), db)) {
  ...
  if (form.dataset.form === 'admin-project-create') {
    ...
    const details = window.MMProjectAdmin.detailsFrom(values);
    const error = window.MMProjectAdmin.validDetails(details);
    if (error) { toast(error); return; }
    const project = {
      id: 'P-' + number.replace(/\D/g, ''), number: number, name: values.get('name'), customer: values.get('customer'),
      address: values.get('address'), contact: values.get('contact'),
      tasks: String(values.get('tasks')).split(/\r?\n/).map(function (item) { return item.trim(); }).filter(Boolean),
      status: 'Neu angelegt', kind: 'ok', open: [], active: true, manager: values.get('manager'), ...details
    };
    ...
    if (db.projectAccounts) db.projectAccounts.push({ site: number, project: project.name, manager: project.manager, offerNet: details.offerNet, plannedHours: details.plannedHours, acceptedHours: 0, travelHoursRaw: 0, traineeHours: 0, materials: 0, lift: 0, subcontractor: 0, tempStaff: 0, scaffoldWaste: 0, other: 0, carryover: 0 });
    audit('PROJECT_CREATED', number, 'Projekt angelegt', '–', 'Bau-Nr. ' + number + ' · ' + project.name, 'Synthetische Neuanlage in der Verwaltung');
    saveDb(); render(); toast('Projekt ' + number + ' wurde angelegt und protokolliert.'); return;
  }
  if (form.dataset.form === 'admin-project-edit') {
    const project = site(form.dataset.id);
    const details = window.MMProjectAdmin.detailsFrom(values, project);
    const error = window.MMProjectAdmin.validDetails(details);
    if (error) { toast(error); return; }
    const before = JSON.stringify({ name: project.name, customer: project.customer, address: project.address, contact: project.contact, manager: project.manager, tasks: project.tasks, details: { access: project.access, siteLead: project.siteLead, dates: project.dates, description: project.description, extraTasks: project.extraTasks, materials: project.materials, invoice: project.invoice, internalNote: project.internalNote } });
    Object.assign(project, { name: values.get('name'), customer: values.get('customer'), address: values.get('address'), contact: values.get('contact'), manager: values.get('manager'), tasks: String(values.get('tasks') || '').split(/\r?\n/).map(function (item) { return item.trim(); }).filter(Boolean), ...details });
    const account = db.projectAccounts && db.projectAccounts.find(function (item) { return item.site === project.number; });
    if (account) Object.assign(account, { project: project.name, manager: project.manager, offerNet: details.offerNet, plannedHours: details.plannedHours });
    audit('PROJECT_UPDATED', project.number, 'Projektstammdaten geändert', before, JSON.stringify({ name: project.name, customer: project.customer, address: project.address, contact: project.contact, manager: project.manager, tasks: project.tasks, details: details }), 'Bearbeitung in der Verwaltung');
  ...
  if (form.dataset.form === 'admin-employee-create') {
    ...
    const employee = { id: id, name: values.get('name'), job: values.get('job'), role: values.get('role'), status: plannedSite ? 'NOT_STARTED' : 'NOT_PLANNED', site: plannedSite, plannedSite: plannedSite, since: 'noch nicht', active: true, createdAt: dateTimeNow(), createdBy: actor(), demoPin: String(100000 + highest + 1) };
